...
function getN1Area(person) {
    ...
    return person.n1 || person.diretoria || person.departamento || person.tipo_vinculo || 'OUTROS';
}
...
function createBooking(person, origin, dest, serviceType, accompany, date, time, canal_entrada) {
...
function handleLegReplication(prefix, leg, person, origin, dest, serviceType, accompany, selectedDate, time, canal) {
    ...
    targetDates.forEach(d => {
        const ok = createBooking(person, origin, dest, serviceType, accompany, d, time, canal);
        if (ok) count++;
    });
...
function handlePreBookingSubmit() {
    ...
    if (conflictingDates.length > 0) {
        const confirmMsg = `Atenção: Você já possui agendamento de transporte ativo para o(s) dia(s) ${conflictingDates.join(', ')}.\nDeseja alterar a opção selecionada anteriormente por estas novas?`;
...
function refreshOperationList() {
    ...
    const capMax = trip ? trip.capacidade : (serviceType === 'Vai e Vem Van' ? 15 : 4);
...
function renderAdherenceReport() {
    ...
    top10.forEach(g => {
        ...
        tr.innerHTML = `
            <td class="py-3 px-4 font-bold text-gray-300">${g.name}</td>
    ...
    // Renderiza também a tabela de validação de programações CCO
...
function renderAuditValidationTable() {
    ...
    const bookings = db.bookings.filter(b => activeDates.includes(b.data) && b.status !== 'Cancelado');
    ...
    sortedAreas.forEach(g => {
        ...
        const btnAction = g.valid 
            ? `<button disabled class="bg-gray-800 text-gray-600 border border-gray-700 px-3 py-1 rounded-xl text-[10px] font-bold uppercase cursor-not-allowed flex items-center gap-1 shadow-md"><i class="fa-solid fa-check mr-0.5"></i>Validado</button>`
        ...
        tr.className = "hover:bg-gray-900/20 transition-colors";
        tr.innerHTML = `
            <td class="py-3 px-4 font-bold text-gray-300">${g.area}</td>
...
function validateN1Program(areaName) {
    const activeDates = getEventDates();
...
function exportAdherenceCSV() {
    ...
    eventBookings.forEach(b => {
        ...
        } else if (b.status === 'No-Show' || b.status === 'Agendado') {
            groups[n1].noshow += 1;
            groups[n1].planejado += 1;
        }
    });
...
function recalculateFinancialLoss() {
    ...
    eventBookings.forEach(b => {
        ...
        if (b.status === 'No-Show' || b.status === 'Agendado') {
            groups[n1].noshow += 1;
...
function processBookingRawData(rows) {
    ...
    let matrIdx = -1, cpfIdx = -1, nameIdx = -1, telIdx = -1, dirIdx = -1, deptIdx = -1, cargoIdx = -1, origIdx = -1, destIdx = -1, dateIdx = -1, timeIdx = -1, serviceIdx = -1, accIdx = -1, replIdx = -1;

    const isHeader = headers.some(h => h.includes('MATRICULA') || h.includes('CPF') || h.includes('ORIGEM') || h.includes('DESTINO'));
    let startRow = 0;

    if (isHeader) {
        startRow = 1;
        headers.forEach((h, idx) => {
            if (h.includes('MATRICULA')) matrIdx = idx;
            else if (h.includes('CPF')) cpfIdx = idx;
            else if (h.includes('NOME')) nameIdx = idx;
            else if (h.includes('TELEFONE') || h.includes('TEL')) telIdx = idx;
            else if (h.includes('DIRETORIA')) dirIdx = idx;
            else if (h.includes('DEPARTAMENTO') || h.includes('DEP')) deptIdx = idx;
            else if (h.includes('CARGO') || h.includes('FUN')) cargoIdx = idx;
            else if (h.includes('ORIGEM') || h.includes('ORIG')) origIdx = idx;
            else if (h.includes('DESTINO') || h.includes('DEST')) destIdx = idx;
            else if (h.includes('DATA')) dateIdx = idx;
            else if (h.includes('HORA') || h.includes('TEMPO')) timeIdx = idx;
            else if (h.includes('ATENDIMENTO') || h.includes('SERVICO') || h.includes('TIPO')) serviceIdx = idx;
            else if (h.includes('ACOMPANHANTE')) accIdx = idx;
            else if (h.includes('REPLICAR')) replIdx = idx;
        });
    } else {
        matrIdx = 0; cpfIdx = 1; nameIdx = 2; telIdx = 3; dirIdx = 4; deptIdx = 5; cargoIdx = 6; origIdx = 7; destIdx = 8; dateIdx = 9; timeIdx = 10; serviceIdx = 11; accIdx = 12; replIdx = 13;
    }
    ...
    for (let i = startRow; i < rows.length; i++) {
        ...
        const matricula = String(row[matrIdx] || '').trim().split('.')[0];
        const cpf = String(row[cpfIdx] || '').trim().replace(/\\D/g, '');
        const nome = String(row[nameIdx] || '').trim();
        const telefone = String(row[telIdx] || '').trim();
        const diretoria = String(row[dirIdx] || '').trim();
        const departamento = String(row[deptIdx] || '').trim();
        const cargo = String(row[cargoIdx] || '').trim();
        const origem = String(row[origIdx] || '').trim().toUpperCase();
        const destino = String(row[destIdx] || '').trim().toUpperCase();
        const data_viagem = String(row[dateIdx] || '').trim();
        const hora_viagem = String(row[timeIdx] || '').trim();
        const serviceType = String(row[serviceIdx] || '').trim();
        const accompany = String(row[accIdx] || '').trim();

        let person = findPerson(matricula) || findPerson(cpf);
        if (!person && nome.length >= 3) {
            person = {
                matricula: matricula || '',
                nome: nome,
                cpf: cpf || '',
        ...
        const enableVai = destino === 'SAMBODROMO' || destino === 'CIDADE DO ROCK' || destino === 'ROCK IN GLOBO' || destino === 'EVENTO' || destino === 'ROCK IN RIO';
        const enableVem = origem === 'SAMBODROMO' || origem === 'CIDADE DO ROCK' || origem === 'ROCK IN GLOBO' || origem === 'EVENTO' || origem === 'ROCK IN RIO';

        const vaiTime = enableVai ? hora_viagem : '';
        const vemTime = enableVem ? hora_viagem : '';
        
        const vaiOrigin = enableVai ? (origem || 'EG') : 'EG';
        const vemDest = enableVem ? (destino || 'EG') : 'EG';
        ...
        pendingBookings.push({
            ...
            vemDest: vemDest,
            vemTime: vemTime,
            serviceType: serviceType || 'Vai e Vem Van',
            accompany: accompany,
            canal: 'Excel'
        });
    }
    ...
    if (skipped > 0) {
        showToast("Agendamentos em Lote", `${pendingBookings.length} processados, ${skipped} pulados por falta de dados do colaborador.`, "warning");
    }
}
...
function downloadBookingTemplate() {
    const filename = "modelo_agendamento_lote.xls";
    const headers = ["MATRICULA", "CPF", "NOME_COMPLETO", "TELEFONE", "DIRETORIA", "DEPARTAMENTO", "CARGO", "ORIGEM", "DESTINO", "DATA_VIAGEM", "HORA_VIAGEM", "TIPO_ATENDIMENTO", "ACOMPANHANTES"];
    const rows = [
        ["68808", "8018382743", "JULIANA GONCALVES DA SILVA", "21999999999", "Suprimentos Serviços e Logística", "Transportes RJ", "LÍDER", "EG", "Sambodromo", "13/09/2024", "21:00", "Vai e Vem Van Coletivo", ""]
    ];
    generateStyledExcel(filename, headers, rows);
}
...
function switchFleetSubTab(subTabId) {
    ...
    const subtabs = ['drivers', 'vehicles', 'companies'];
    subtabs.forEach(s => {
...
function saveFleetRecord() {
    ...
    if (type === 'driver') {
        ...
        const payload = { placa, modelo, tipo, capacidade, empresa, status };
        ...
        slides: [
            {
                ...
                desc: "Acesse a aba <strong>'Portal de Pré-Agendamento'</strong> usando sua matrícula ou CPF. O sistema identificará seu cadastro básico e permitirá que você escolha a data e o tipo de viagem (Ida e/ou Volta).",
                badge: "Etapa 1",
                mockup: `
                    <div class="bg-gray-900/60 border border-blue-500/20 rounded-xl p-3 space-y-1.5 text-left text-[11px] font-sans">
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-1">
                            <div class="bg-gray-950 p-1.5 rounded border border-gray-800">
                                <span class="text-gray-500 block text-[8px] uppercase">CPF/Matrícula</span>
                                <span class="text-white font-semibold">123.456.789-00</span>
                            </div>
                            <div class="bg-gray-950 p-1.5 rounded border border-gray-850">
                                <span class="text-gray-500 block text-[8px] uppercase">Passageiro</span>
                                <span class="text-white font-semibold">Colaborador Globo</span>
                            </div>
                        </div>
                        <div class="bg-blue-500/5 border border-blue-500/10 p-1.5 rounded text-blue-300 text-[10px]">
                            <i class="fa-solid fa-check-circle mr-1"></i> Cadastro validado com sucesso na base CCO.
                        </div>
                    </div>
                `
            },
    ...
    manager: {
        ...
        slides: [
            ...
            },
            {
                ...
                desc: "Na aba <strong>'Administração de Acessos'</strong>, audite conexões ativas. Você visualiza detalhes de quem está logado no sistema e tem o poder de **derrubar sessões de usuários remotamente**, forçando o logout imediato para preservar dados do CCO.",
                badge: "Etapa 3",
                mockup: `
                    <div class="bg-gray-900/60 border border-amber-500/20 rounded-xl p-3 text-left text-[11px] space-y-2">
                        <div class="text-[10px] text-amber-400 font-bold uppercase tracking-wider">Controle de Sessões Ativas</div>
                        <div class="bg-gray-950 p-2 rounded border border-gray-800 flex justify-between items-center text-[10px]">
                            <div>
                                <span class="text-white font-semibold">User: operador_02</span>
                                <span class="text-gray-500 block text-[9px]">Origem: 192.168.1.5</span>
                            </div>
                            <button class="bg-red-600 hover:bg-red-500 text-white font-bold px-2 py-0.5 rounded text-[8px] uppercase">Derrubar</button>
                        </div>
                    </div>
                `
            },
